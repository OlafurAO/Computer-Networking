                  ...
                  char *buffer)
{
  ...
  if((tokens[0].compare("SYS") == 0) && (tokens.size() >= 2))
  {
    ...
    cmd << tokens[1].c_str() << " > command_output.txt";
    system(cmd.str().c_str()); // Execute the command and write the output into a .txt file

    std::ifstream outputFile("command_output.txt");
    std::string commandOutput;

    // Go through the .txt file and feed each line into commandOutput
    while(!outputFile.eof()) {
        std::string input;
        std::getline(outputFile, input);
        commandOutput += "\n";
        commandOutput += input;
    }
    ...
    if(commandOutput.size() != 0) {
        ...
        std::string message = "No output available";
  ...
  {
     std::string errorMessage = "    ERROR: Unknown command from client!";
